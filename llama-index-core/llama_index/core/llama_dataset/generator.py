...
RE_SPLIT_QUESTION = re.compile(r"^\d+[\).\s]")
...
class RagDatasetGenerator(PromptMixin):
    ...
    def __init__(
        ...
        """Init params."""
        llm_final = llm or llm_from_settings_or_context(Settings, service_context)
        self._llm = llm_final
        self.text_question_template = (
            text_question_template
            if text_question_template is not None
            else PromptTemplate(DEFAULT_QUESTION_GENERATION_PROMPT)
        )
    ...
    async def _agenerate_dataset(
        ...
        query_tasks = []
        make_doc = partial(self._make_doc_from_node, metadata_mode=self._metadata_mode)
        # Create all indices and engines up front to minimize code cost in the loop
        node_docs = [make_doc(node) for node in nodes]
        summary_indices: List[SummaryIndex] = [
            SummaryIndex.from_documents([doc]) for doc in node_docs
        ]
        create_query_engine = partial(
            lambda idx: idx.as_query_engine(
                llm=self._llm,
                text_qa_template=self.text_question_template,
                use_async=True,
            )
        )
        # Pre-create question generation engines (can be reused for each node chunk)
        query_engines = [create_query_engine(idx) for idx in summary_indices]
        for qe in query_engines:
            task = qe.aquery(self.question_gen_query)
            query_tasks.append(task)
        responses = await run_jobs(query_tasks, self._show_progress, self._workers)
        # Pre-extract context and index mapping for future use
        ...
        # If not labelled, create all examples efficiently
        if not labelled:
            for idx, response in enumerate(responses):
                result = str(response).strip().split("\n")
                cleaned_questions = self._clean_questions(result)
                for query in cleaned_questions:
                    example = LabelledRagDataExample(
                        query=query,
                        reference_answer="",
                        reference_contexts=[node_texts[idx]],
        ...
        # Labelled: do Q&A for each question (expensive)
        # Prepare qr_tasks in large batches per node, schedule all at once per node chunk
        for idx, response in enumerate(responses):
            result = str(response).strip().split("\n")
            cleaned_questions = self._clean_questions(result)
            index = summary_indices[idx]
            reference_context = node_texts[idx]
            # Prepare QA query engines (cache prompt to avoid object re-creation)
            qa_query_engine = index.as_query_engine(
                llm=self._llm, text_qa_template=self.text_qa_template
            )
            if cleaned_questions:
                qr_tasks = [qa_query_engine.aquery(query) for query in cleaned_questions]
    ...
    def _clean_questions(result: List[str]) -> List[str]:
        # Use the pre-compiled pattern and combine strip within listcomp
        cleaned = [RE_SPLIT_QUESTION.sub("", question).strip() for question in result]
        return [q for q in cleaned if q]

    @staticmethod
    def _make_doc_from_node(node: BaseNode, metadata_mode: MetadataMode) -> Document:
        # Inline Document creation and extraction for reuse and clarity (faster than repeatedly passing the same keys)
        return Document(
            text=node.get_content(metadata_mode=metadata_mode),
