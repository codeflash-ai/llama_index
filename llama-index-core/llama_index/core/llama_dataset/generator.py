...
class RagDatasetGenerator(PromptMixin):
    ...
    def __init__(
        ...
        # Pre-store prompt dict for faster retrieval in _get_prompts
        text_question_template_final = (
            text_question_template
            if text_question_template is not None
            else PromptTemplate(DEFAULT_QUESTION_GENERATION_PROMPT)
        )
        text_qa_template_final = (
            text_qa_template if text_qa_template is not None else DEFAULT_TEXT_QA_PROMPT
        )
        self._prompt_dict: PromptDictType = {
            "text_question_template": text_question_template_final,
            "text_qa_template": text_qa_template_final,
        }
        self.text_question_template = text_question_template_final
        self.text_qa_template = text_qa_template_final
    ...
    def _get_prompts(self) -> PromptDictType:
        """Get prompts."""
        # Return the stored prompt dictionary directly for efficiency.
