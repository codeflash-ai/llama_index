...
class NLStructStoreQueryEngine(BaseQueryEngine):
    ...
    def _get_table_context(self, query_bundle: QueryBundle) -> str:
        ...
        """
        context_container = self._sql_context_container
        context_str = context_container.context_str
        if context_str is not None:
            return context_str

        context_dict = context_container.context_dict
        if context_dict is None:
            raise ValueError(
                "context_dict must be provided. There is currently no "
                "table context."
            )
        # Preallocating list, especially for large dicts
        # Use generator expression + join for maximum efficiency and reduced memory spikes
