...
def default_output_processor(
    ...
    try:
        ...
        # Split tree.body only once for efficiency
        body, last = tree.body[:-1], tree.body[-1:]
        # Only call ast.Module and ast.unparse if body is non-empty
        if body:
            module = ast.Module(body, type_ignores=[])
            safe_exec(ast.unparse(module), {}, local_vars)  # type: ignore
        module_end = ast.Module(last, type_ignores=[])
        ...
        try:
            ...
            if "max_colwidth" in output_kwargs:
                # pd.option_context avoids global state changes and is safer/faster than set/reset
                with pd.option_context("display.max_colwidth", output_kwargs["max_colwidth"]):
