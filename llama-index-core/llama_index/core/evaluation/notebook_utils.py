...
def get_retrieval_results_df(
    ...
    # Pre-compute all metric dictionaries and DataFrames
    all_metric_dicts = [
        [eval_result.metric_vals_dict for eval_result in eval_results]
        for eval_results in results_arr
    ]
    results_dfs = [pd.DataFrame(metric_dicts) for metric_dicts in all_metric_dicts]

    for name, results_df in zip(names, results_dfs):
